...
try {
 for(const viewport of viewports){
  ...
  await page.waitForFunction(()=>Number(document.querySelector('.experience').dataset.journeyProgress)>.93);await page.waitForTimeout(800);
  await page.screenshot({path:`${folder}/${viewport.width}-entry.png`});
  const originalY=await page.evaluate(()=>scrollY);
  await page.locator('[data-story="3"] [data-brew-start]').click();
  await page.waitForFunction(()=>document.querySelector('.brew-dialog')?.dataset.ready==='true');
  ...
  const stage=async(step,name)=>{
   await page.waitForFunction(step=>document.querySelector('.brew-dialog')?.dataset.step===String(step),step);
   await page.waitForTimeout(300);
   assert.equal(await page.evaluate(()=>document.querySelector('.brew-dialog').scrollWidth>innerWidth),false);
   await page.screenshot({path:`${folder}/${viewport.width}-${name}.png`});
  };
  await stage(0,'assembly');
  assert.equal(await page.locator('canvas').count(),2,'The workbench is created only on demand');
  ...
  await page.keyboard.press('Escape');await page.waitForFunction(()=>!document.querySelector('.brew-dialog'));
  assert.equal(await page.locator('canvas').count(),1,'Closing disposes the additional scene');
  assert.ok(Math.abs(await page.evaluate(()=>scrollY)-originalY)<2,'Closing preserves the story position');
  assert.ok(await page.evaluate(()=>document.activeElement.matches('[data-brew-start]')),'Focus returns to the entry button');
  assert.notEqual(await page.evaluate(()=>document.documentElement.style.overflow),'hidden');
  report.push({viewport,water,pressure,closed:true,replayed:true});await page.close();console.log('PASS brewing',viewport);
 }
